...
import { isOfficialFsanzImageUrl, isOfficialFsanzUrl } from './normalize-fsanz-food-recalls.ts';
...
type AuditSummary = {
  ...
  recordsWithDistributionTerms: number;
...
const processedPath = resolve(projectRoot, 'data/processed/fsanz-food-recalls.json');
...
async function readProcessedFsanzRecords(): Promise<NormalizedRecall[]> {
  const text = await readFile(processedPath, 'utf8');
  const payload = JSON.parse(text) as ProcessedRecallFile;
  return Array.isArray(payload.records) ? payload.records.filter((record) => record.source === 'FSANZ_FOOD_RECALLS') : [];
}
...
function nonOfficialImageHostIssues(records: NormalizedRecall[]): AuditIssue[] {
  return records.flatMap((record) =>
    imageUrlsFor(record)
      .filter((url) => !isOfficialFsanzImageUrl(url))
      .map((url) => compactIssue(record, url))
  );
}
...
function suspiciousImageCandidateIssues(records: NormalizedRecall[]): AuditIssue[] {
  const suspiciousPattern =
    /\b(?:logo|favicon|sprite|icon|social|share|tracking|analytics|header|footer|placeholder|avatar|banner|og-image|inline-images)\b/i;
...
function hasIdentifierLikeValue(record: NormalizedRecall): boolean {
  ...
  return [
    ...
    /\b\d{8,14}\b/,
    /\b(?:date marking|best before|best-before|use-by|use by|expiry|expiration)\b/i,
    /\b(?:pack size|net weight|\d+\s*(?:g|kg|ml|l|litre|litres))\b/i
...
function hasDistributionTerm(record: NormalizedRecall): boolean {
  return /\b(National|NSW|VIC|QLD|WA|SA|TAS|ACT|NT|Australia|New Zealand|supermarket|retailer|available for sale|online)\b/i.test(
    textFieldsFor(record)
  );
}
...
function audit(records: NormalizedRecall[]): AuditSummary {
  ...
  const wrongSourceIds = records
    .filter((record) => record.source !== 'FSANZ_FOOD_RECALLS')
  ...
  const warnings = [
    recordsWithImages.length === 0
      ? 'FSANZ food recall pages in the bounded sample did not expose official product image URLs; existing fallback image behavior is expected.'
      : '',
    countMissing(records, (record) => record.affectedUnits.length === 0) > 0
      ? 'Some FSANZ notices do not expose batch, barcode, pack, or date identifiers in a consistent structured field.'
      : '',
    suspiciousImageCandidates.length > 0
      ? `Suspicious FSANZ image candidate strings found: ${suspiciousImageCandidates.length}; review before accepting future refreshes.`
      : ''
  ].filter(Boolean);
  ...
  return {
    ...
    nonOfficialSourceUrls: records.filter((record) => !isOfficialFsanzUrl(record.sourceUrl)).map((record) => compactIssue(record, record.sourceUrl)),
    missing: {
    ...
    recordsWithDistributionTerms: records.filter(hasDistributionTerm).length,
...
function buildBlockers(summary: AuditSummary, canonicalCounts: SourceCounts, sourceFilters: string[]): string[] {
  ...
  return [
    summary.total === 0 ? 'FSANZ_FOOD_RECALLS count is 0.' : '',
    summary.total !== expectedCounts.FSANZ_FOOD_RECALLS
      ? `FSANZ_FOOD_RECALLS count ${summary.total} does not match expected ${expectedCounts.FSANZ_FOOD_RECALLS}.`
      : '',
    ...
    canonicalCounts.FSANZ_FOOD_RECALLS !== expectedCounts.FSANZ_FOOD_RECALLS
      ? `Canonical FSANZ_FOOD_RECALLS count ${canonicalCounts.FSANZ_FOOD_RECALLS} does not match expected ${expectedCounts.FSANZ_FOOD_RECALLS}.`
      : '',
    summary.wrongSourceIds.length > 0 ? `Wrong source ids found in FSANZ file: ${summary.wrongSourceIds.length}.` : '',
    ...
    summary.missing.remedyOrAction > severeMissingThreshold
      ...
      : '',
    summary.invalidRecallDates.length > 0 ? `Invalid FSANZ recall dates found: ${summary.invalidRecallDates.length}.` : '',
    summary.nonOfficialSourceUrls.length > 0 ? `Non-official FSANZ source URLs found: ${summary.nonOfficialSourceUrls.length}.` : '',
    summary.invalidImageUrls.length > 0 ? `Invalid FSANZ image URLs found: ${summary.invalidImageUrls.length}.` : '',
...
async function runAudit(): Promise<void> {
  const records = await readProcessedFsanzRecords();
  if (records.length === 0) {
    throw new Error(`No FSANZ_FOOD_RECALLS records found in ${processedPath}`);
  }
    ...
    JSON.stringify(
      {
        ...
        auditSummary: {
          ...
          fsanzFoodRecallCount: summary.total,
        ...
        operationalSummary: {
          ...
          recordsWithDistributionTerms: summary.recordsWithDistributionTerms,
