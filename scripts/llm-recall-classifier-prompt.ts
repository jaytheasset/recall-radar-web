...
export function buildRecallClassifierPromptFromInput(input: unknown, model = '', inputLabel = 'Recall input'): string {
  ...
  return [
    ...
    'For beverages, frozen confection or dairy desserts, packaged foods, ingredients, supplements sold as food, infant formula, allergens, pathogens, foreign matter in food, or food date/lot recalls: productFamily must be food-grocery and recallDomain must be food.',
    ...
    'Use the closest allowed productType instead of unknown when the product text is clear: smoke alarms/detectors and carbon monoxide alarms/detectors -> electronics-other; lamps and night lights -> lighting; mobile, cell, feature, or candybar phones -> electronics-other; electronic headsets, hearing defenders, and speaker-based hearing protection -> electronics-other; waxing kits and wax warmers -> personal-care-product; washing machines -> laundry-appliance; kettles -> kitchen-appliance; extension cords and USB power-supply storage holders -> electronic-accessory; spa/pool drain covers -> pool-water-sports; lighters -> other; scuba or diving regulators -> pool-water-sports; vacuum cleaners -> home-appliance-other.',
    ...
    'Pick one primary hazardType. Use contamination-chemical for chemical contaminants including asbestos. Use regulatory-noncompliance for labeling, certification, or standard-compliance failures when no concrete injury mechanism is stated.',
